    ...
    <div className="h-auto w-96 min-h-full bg-gray-200 p-8 border border-2 border-gray-300 drop-shadow-2xl rounded">
      <div>
        ...
        <div className="font-sans flex items-center justify-center bg-blue-darker w-full py-8 drop-shadow-2xl">
          <div className="overflow-hidden bg-white rounded max-w-xs w-full shadow-lg  leading-normal bg-red-400 cursor-pointer" onClick={() => {
            console.log('Test')
          }}>
            <div className="block group hover:bg-blue p-4 border-b">
              <p className="font-bold text-lg mb-1 text-black ">{header}</p>
              {subSections?.length !== 0 && (
                <p className="text-grey-darker mb-2 ">{`test`}</p>
              )}
            </div>
          </div>
        </div>
